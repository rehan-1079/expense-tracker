...
function updateTransactionById(id, data) {
  for (let i = 0; i < transactions.length; i++) {
    if (transactions[i].id === id) {
      transactions[i].name = data.name;
      transactions[i].amount = Number(data.amount);
...
function renderTransactionList() {
  ...
  if (visible.length === 0) {
    ...
    // Message changes slightly depending on whether there are ANY transactions at all
...
transactionForm.addEventListener("submit", function (e) {
  ...
  if (editId) {
    updateTransactionById(editId, data);
  } else {
    addTransaction(data);
  }
...
confirmDeleteBtn.addEventListener("click", function () {
  if (deleteId) {
    deleteTransactionById(deleteId);
    deleteId = null;
  }
  closeModal(deleteModal);
});
